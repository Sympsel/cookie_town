...
(function () {
  ...
  async function refreshSession() {
    if (!getToken()) { renderSessionStatus(); return null; }
    try {
  ...
  function resolveEndpoint(form) {
    ...
    const qp = (form.dataset.queryParams || '').split(',').map(function (s) { return s.trim(); }).filter(Boolean);
    if (qp.length) {
      const parts = [];
    ...
    const nf = (form.dataset.numberFields || '').split(',').map(function (s) { return s.trim(); }).filter(Boolean);
  ...
  function buildForm(cfg) {
    ...
    (cfg.fields || []).forEach(function (f) { form.appendChild(fieldToInput(f, idBase)); });
  ...
  function buildPanel(p) {
    ...
    det.addEventListener('toggle', async function () {
      if (det.open && !loaded) { loaded = true; await renderPanelList(p, listBox); }
    });
    return det;
  }

  async function renderPanelList(p, box) {
    if (!p.load) { box.remove(); return; }
    box.textContent = '加载中…';
    try {
      ...
      data.forEach(function (item) { box.appendChild(p.load.render(item)); });
    } catch (e) {
      box.textContent = '';
  ...
  // 子资源项渲染器
  function userChip(u) { return tile(u.uuid, [['用户', u.name], ['权限', u.permission], ['UUID', u.uuid]]); }
  function coordRow(c) { return tile(null, [['坐标', '(' + c.x + ', ' + c.y + ', ' + c.z + ')'], ['维度', c.level]]); }
  function textRow(s) { const d = el('div', 'tile tile-item'); d.appendChild(el('span', 'tile-val', String(s))); return d; }
  function townChip(t) { return tile(t.uuid, [['名称', t.name], ['UUID', t.uuid]]); }
  function landmarkChip(l) { return tile(l.uuid, [['地标', l.name], ['类型', l.type], ['UUID', l.uuid]]); }
  function commentChip(c) { return tile(c.uuid, [['内容', c.content], ['发布者', c.publisherUuid], ['UUID', c.uuid]]); }
  ...
  const TILE_BUILDERS = {
    '/api/towns': function (t) {
      ...
      actions.appendChild(wrapInDetails('编辑', buildForm({
        title: '编辑', endpoint: '/api/towns/' + t.uuid, method: 'PUT', submitLabel: '保存', requirePermission: 'Common',
        fields: [
      ...
      node.appendChild(buildPanel({
        ...
        forms: [
          { title: '添加成员', endpoint: '/api/towns/' + t.uuid + '/members/{userUuid}', method: 'POST', submitLabel: '添加', requirePermission: 'Common', fields: [{ name: 'userUuid', label: '用户 UUID', type: 'text', required: true }] },
          { title: '移除成员', endpoint: '/api/towns/' + t.uuid + '/members/{userUuid}', method: 'DELETE', submitLabel: '移除', requirePermission: 'Common', fields: [{ name: 'userUuid', label: '用户 UUID', type: 'text', required: true }] }
        ]
      }));
      node.appendChild(buildPanel({ title: '子城镇', load: { endpoint: '/api/towns/' + t.uuid + '/children', render: townChip } }));
      return node;
    },
    ...
    '/api/landmarks': function (l) {
      ...
      const actions = el('div', 'tile-actions');
      actions.appendChild(wrapInDetails('编辑', buildForm({
        title: '编辑', endpoint: '/api/landmarks/' + l.uuid, method: 'PUT', submitLabel: '保存', requirePermission: 'Common',
        fields: [
      ...
      actions.appendChild(wrapInDetails('改状态', buildForm({
        title: '改状态', endpoint: '/api/landmarks/' + l.uuid + '/status', method: 'PUT', submitLabel: '更新状态', requirePermission: 'Common', query: ['status'],
      ...
      const coordFields = [
        ...
        { name: 'level', label: '维度', value: 0, type: 'text', required: true, hint: '-1 下界 / 0 主世界 / 1 末地' }
      ];
      node.appendChild(buildPanel({
        title: '建造者', load: { endpoint: '/api/landmarks/' + l.uuid + '/builders', render: userChip },
        forms: [
          { title: '添加建造者', endpoint: '/api/landmarks/' + l.uuid + '/builders/{userUuid}', method: 'POST', submitLabel: '添加', requirePermission: 'Common', fields: [{ name: 'userUuid', label: '用户 UUID', type: 'text', required: true }] },
          { title: '移除建造者', endpoint: '/api/landmarks/' + l.uuid + '/builders/{userUuid}', method: 'DELETE', submitLabel: '移除', requirePermission: 'Common', fields: [{ name: 'userUuid', label: '用户 UUID', type: 'text', required: true }] }
        ]
      }));
      node.appendChild(buildPanel({
        title: '坐标', load: { endpoint: '/api/landmarks/' + l.uuid + '/coordinates', render: coordRow },
        forms: [
          { title: '添加坐标', endpoint: '/api/landmarks/' + l.uuid + '/coordinates', method: 'POST', submitLabel: '添加', requirePermission: 'Common', numbers: ['x', 'y', 'z', 'level'], fields: coordFields },
          { title: '移除坐标', endpoint: '/api/landmarks/' + l.uuid + '/coordinates', method: 'DELETE', submitLabel: '移除', requirePermission: 'Common', numbers: ['x', 'y', 'z', 'level'], fields: coordFields }
        ]
      }));
      node.appendChild(buildPanel({
        title: '图片', load: { endpoint: '/api/landmarks/' + l.uuid + '/pictures', render: textRow },
        forms: [
          { title: '添加图片', endpoint: '/api/landmarks/' + l.uuid + '/pictures', method: 'POST', submitLabel: '添加', requirePermission: 'Common', query: ['url'], fields: [{ name: 'url', label: '图片 URL', type: 'text', required: true }] },
          { title: '移除图片', endpoint: '/api/landmarks/' + l.uuid + '/pictures', method: 'DELETE', submitLabel: '移除', requirePermission: 'Common', query: ['url'], fields: [{ name: 'url', label: '图片 URL', type: 'text', required: true }] }
        ]
      }));
      node.appendChild(buildPanel({ title: '子地标', load: { endpoint: '/api/landmarks/' + l.uuid + '/children', render: landmarkChip } }));
      return node;
    },
    ...
    '/api/notices': function (n) {
      ...
      const actions = el('div', 'tile-actions');
      actions.appendChild(wrapInDetails('编辑', buildForm({
        title: '编辑', endpoint: '/api/notices/' + n.uuid, method: 'PUT', submitLabel: '保存', requirePermission: 'Admin',
        fields: [
    ...
    '/api/message-boards': function (m) {
      ...
      const actions = el('div', 'tile-actions');
      actions.appendChild(wrapInDetails('编辑', buildForm({
        title: '编辑', endpoint: '/api/message-boards/' + m.uuid, method: 'PUT', submitLabel: '保存', requirePermission: 'Common',
        fields: [
      ...
      node.appendChild(actions);
      node.appendChild(buildPanel({ title: '回复', load: { endpoint: '/api/message-boards/' + m.uuid + '/replies', render: textRow } }));
      return node;
    },
    ...
    '/api/comments': function (c) {
      ...
      const actions = el('div', 'tile-actions');
      actions.appendChild(wrapInDetails('编辑', buildForm({
        title: '编辑', endpoint: '/api/comments/' + c.uuid, method: 'PUT', submitLabel: '保存', requirePermission: 'Common',
      ...
      node.appendChild(buildPanel({
        ...
        forms: [{
          title: '发表回复', endpoint: '/api/comments', method: 'POST', submitLabel: '回复', requirePermission: 'Common',
          fields: [
    ...
    '/api/users': function (u) {
      ...
      actions.appendChild(wrapInDetails('改资料', buildForm({
        title: '改资料', endpoint: '/api/users/' + u.uuid, method: 'PUT', submitLabel: '保存', requirePermission: 'Common',
        fields: [
      ...
      actions.appendChild(wrapInDetails('改权限', buildForm({
        title: '改权限', endpoint: '/api/users/' + u.uuid + '/permission', method: 'PUT', submitLabel: '更新权限', requirePermission: 'Admin', query: ['permission'],
      ...
      node.appendChild(actions);
      node.appendChild(buildPanel({ title: '标签', load: { endpoint: '/api/users/' + u.uuid + '/tags', render: textRow } }));
      return node;
    }
  };
  ...
  async function loadList(container) {
    ...
    try {
      ...
      if (!Array.isArray(data) || data.length === 0) {
        container.appendChild(el('p', 'empty-hint', '暂无数据。'));
        return;
      }
      ...
      container.appendChild(el('p', 'empty-hint error', '加载失败：' + e.message));
    }
  }
  ...
  function applyPermissionGating() {
    ...
    document.querySelectorAll('section.form-section form.tile-form').forEach(function (form) {
      ...
      showMsg(form, ok ? '' : can0Hint(required), !ok);
    });
  }
